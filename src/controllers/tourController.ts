...
export const getAllTours = async (req: Request, res: Response) => {
  const tours = await readFile<Tour[]>(TOURS_FILE_PATH);
  res.status(200).json({
    status: 'success',
...
export const createTour = async (req: Request, res: Response) => {
  ...
  const newTour: Tour = { id: newId, ...rest };

  tours.push(newTour);
  await writeFile<Tour[]>(TOURS_FILE_PATH, tours);
...
export const getTourById = async (req: Request, res: Response) => {
  ...
  const tours = await readFile<Tour[]>('tours-simple.json');
